...
numRows = []
for col in rows:
    numRows.append([int(i) for i in col.split(' ')])

best = 0

# for each row, calculate the product in each row, column, and diag
for row in range(len(numRows) - 3):

    for col in range(len(numRows[0]) - 3):
        # diagonal product
    ...
    for col in range(len(numRows[0])):
        # col product
        colProd = numRows[row][col] * numRows[row + 1][col] * numRows[row + 2][col] * numRows[row + 3][col]
        if colProd > best:
            best = colProd
